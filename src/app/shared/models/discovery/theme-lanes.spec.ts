...
describe('theme lanes', () => {
  ...
  const expansion = buildVocabularyExpansion([ring]);
  ...
  it('puts a keyword member in Says and a legendary gate in Needs', () => {
    ...
    expect(themeLanesForCard(nazgul, 'the ring tempts you', expansion)).toEqual(['says']);
    expect(themeLanesForCard(grima, 'the ring tempts you', expansion)).toEqual(['needs']);
  });
